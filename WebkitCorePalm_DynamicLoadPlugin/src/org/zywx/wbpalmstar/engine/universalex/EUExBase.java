...
public abstract class EUExBase {
	...
	protected EUExResource mResourceHelper;
    ...
    public EUExBase(Context context, EBrowserView inParent) {
		...
		mResourceHelper = new EUExResource();
    }
	 ...
	 * @param info
	 */
	public EUExBase(Context context, EBrowserView inParent, PluginInfo info) {
		this(context, inParent);// 引用旧构造函数，兼容
		try {
			mPluginInfo = info;
			if (info.loadType == PluginInfo.TYPE_DYNAMIC) {
				mPluginContext = new PluginContext(mContext, info.classLoader,
						info.assetManager, info.resources, info.packageName);
			}else{
				//如果是静态加载的插件，使用原来的context
				mPluginContext = context;
			}
			mResourceHelper.init(mPluginContext);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
